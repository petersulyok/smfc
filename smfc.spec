...
- Changed: a wrong ipmi_zone= fails at startup on X14/H14 boards. smfc detects
  how many fan zones the board has, so a zone it does not have is rejected
  immediately with a message naming the real number, instead of running until
  the first fan level write fails with a bare IPMI error. smfc-client does not
  warn about a non-FULL fan mode there any more; it reports what is actually
  driving the fans.
