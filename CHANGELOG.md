...
- Platform auto-detection improved: when `platform_name=auto`, the BMC product name is matched by prefix (boards starting with `X14` → `GenericX14Platform`, boards starting with `X9` → `GenericX9Platform`, everything else → `GenericPlatform`).
