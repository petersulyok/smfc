...
### Fixed
- **Fan control works on X14/H14 motherboards.** It did not before: the commands `smfc` sent were wrong, so the fan level never reached the board and the BMC kept running the fans on its own curve while `smfc` reported it had taken over. Alongside making it work, three safeguards now hold on these boards:
