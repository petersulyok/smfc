...
### Changed
- **`smfc` asks the board about itself instead of assuming.** It detects how many fan zones your board has, so an `ipmi_zone=` your board does not have is now rejected at startup with a message naming the real number, instead of failing later with an IPMI error. It also reads which fan modes the board supports.
- `smfc-client` does not warn about a non-FULL fan mode on X14/H14 any more. These boards are not controlled through FULL mode, so any other mode is normal there. The mode is still shown.

### Fixed
- **Fan control works on X14/H14 motherboards.** It did not before: `smfc` reported that it had taken the fans over while the BMC kept running them on its own curve, because the commands it sent were wrong and the fan level never reached the board. If you tried `platform_name=generic_x14` earlier and saw no effect on your fans, this is why.
- **The fans are always handed back to the BMC when `smfc` stops**, `exit_level=-1` included, and also when the exit level cannot be written. Previously they could be left frozen at their last level with nothing regulating them. On these boards the exit level is only a transition: the BMC applies its own curve about a second later, and that curve does not include disk temperatures - see [README chapter 1.6](https://github.com/petersulyok/smfc/blob/main/README.md#16-service-termination).
