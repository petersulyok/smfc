...
- **A fresh install of the RPM package did not enable the `smfc` service**, so it did not start at the next boot. The `%systemd_post` scriptlet runs `systemctl preset smfc.service`, which asks the systemd preset policy whether to enable the unit. The package shipped no preset file, so the catch-all rule of the distribution matched - `disable *` in `90-default.preset` on Fedora and RHEL - and the unit stayed disabled. The package ships `/usr/lib/systemd/system-preset/50-smfc.preset` with the line `enable smfc.service` now, which the `50` prefix puts ahead of the catch-all. The install still does not start the service: review `/etc/smfc/smfc.conf` first, then run `systemctl start smfc`. The rule applies at install time only, so a later `systemctl disable smfc` is permanent, and writing `disable smfc.service` into `/etc/systemd/system-preset/00-smfc.preset` before the install stops the service from being enabled at all. The DEB package was not affected: `deb-systemd-helper enable` enables the unit directly.
