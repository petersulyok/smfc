...
### New
- Advanced multi-segment user-defined control function: temperature-based fan controllers now accept a `control_function=` parameter consisting of a sequence of `temp-level` points, defining an arbitrary piecewise-linear curve (replacing the implicit single linear segment between `min_temp/max_temp` and `min_level/max_level`). When `control_function=` is defined it takes precedence and the `min_temp/max_temp/min_level/max_level` keys in the same section are ignored (and not validated); this is reported at `CONFIG` log level.
...
- New smoke test scenario `shared_zones_cpu_split` (under `test/scenarios/shared_zones_cpu_split.conf`, dispatched via `./test/run_smoke.sh shared_zones_cpu_split`) exercises multi-curve CPU (`CPU:0` + `CPU:1`) combined with cross-family shared-zone arbitration (`CPU:1` + `HD` on zone 1).
- New smoke test scenario `control_function` (under `test/scenarios/control_function.conf`, dispatched via `./test/run_smoke.sh control_function`) exercises the advanced multi-segment user-defined `control_function=` parameter.
...
- `packages.yml` workflow extended to attach `.deb` and `.rpm` artifacts to each GitHub release, and to dispatch a `package-published` event to both `smfc-deb` and `smfc-rpm` repositories so they auto-publish the new version.
- New platform support for **Supermicro X14 motherboards** (`generic_x14`): a dedicated `GenericX14Platform` implementing the X14 BMC's IPMI raw commands — per-zone manual-mode enable/disable, `0x30 0x70 0x88` duty-cycle control using the direct percentage scale (`0x00`–`0x64`), and the extended fan modes `0x00`–`0x0B`. Auto-detected when `platform_name=auto` and the BMC product name starts with `X14`.
...
- Platform auto-detection improved: when `platform_name=auto`, the BMC product name is matched by prefix (boards starting with `X14` → `GenericX14Platform`, `X10QBi` → `X10QBi`, `X9` → `GenericX9Platform`, everything else → `GenericPlatform`).
- `platform_name=` is now validated: only the documented values (`auto`, `generic`, `generic_x9`, `generic_x14`, `X10QBi`) are accepted and any other value raises a configuration error. BMC product-name matching happens exclusively in `auto` mode. The `PlatformName` enum moved into the `Config` class as the single source of truth for these values.
- Platform `platform_name` value `genericx9` renamed to `generic_x9` for naming consistency with the new `generic_x14`. The legacy `genericx9` value is still accepted and normalized automatically, so existing configurations keep working.
- Platform lifecycle API: the abstract `Platform.set_fan_manual_mode()` was replaced by `start()` (called once at startup to prepare manual fan control) and a new `end()` (called once at shutdown to release manual mode and restore automatic BMC control). `end()` is a no-op on platforms that do not need cleanup.
...
- Unit-test suite refactored around the new `Config` class; smoke-test runner reworked accordingly. Broader test-suite modernization completed: the four platform test modules collapsed into a single matrix-driven [`test_platforms.py`](https://github.com/petersulyok/smfc/blob/main/test/test_platforms.py); shared `FanController` contract helpers extracted into [`test_fc_helpers.py`](https://github.com/petersulyok/smfc/blob/main/test/test_fc_helpers.py); legacy `test_data.py` split into [`test_fixtures.py`](https://github.com/petersulyok/smfc/blob/main/test/test_fixtures.py) (TestData + temp-dir fixtures), [`test_mocks.py`](https://github.com/petersulyok/smfc/blob/main/test/test_mocks.py) (pyudev doubles), and [`test_config_builders.py`](https://github.com/petersulyok/smfc/blob/main/test/test_config_builders.py) (six `create_*_config` builders); 14 `run_test_*.sh` wrappers collapsed into one [`run_smoke.sh <scenario>`](https://github.com/petersulyok/smfc/blob/main/test/run_smoke.sh) dispatcher backed by an inline `SCENARIOS` table in [`smoke_runner.py`](https://github.com/petersulyok/smfc/blob/main/test/smoke_runner.py); positional `error_str` arguments replaced by `pytest.param(..., id="...")`; opaque numeric test names renamed to intent-based names (e.g. `test_run_exits_on_bad_args_or_config`, `test_run_happy_path`); `TestData` lifecycle moved into a deterministic `td` pytest fixture.
- All 398 unit-test methods carry a structured docstring template that names the target production function, the mocked classes/functions, the concrete steps performed, and one `ASSERT` bullet per distinct assertion. Five new high-payoff tests added covering: `GpuFc._get_nth_temp()` raising `IndexError` on partial nvidia-smi output, `HdFc._get_nth_temp()` wrapping smartctl rc=1/2 (warning / parse error) as `ValueError`, `Service.run()` propagating an unhandled exception when a controller's `fc.run()` raises mid-loop, `_try_fetch_snapshot()` returning `None` on HTTP 4xx/5xx (`HTTPError` arm), and `_format_report_from_snapshot()` rendering with missing top-level keys (`bmc` / `fan_mode` / `fan_controllers` / `zones`) or an empty snapshot dict. Total: 1000 unit tests passing, 100% statement and branch coverage on `src/smfc/`.
- New testing documentation [`TESTING.md`](https://github.com/petersulyok/smfc/blob/main/TESTING.md) describes the suite organization, design principles, shared infrastructure, source ↔ test mapping, and the smoke scenario matrix. [`DEVELOPMENT.md`](https://github.com/petersulyok/smfc/blob/main/DEVELOPMENT.md) now delegates all test-related material to `TESTING.md`.
...
- README chapter 10.2 sample configuration synchronized with the canonical [`config/smfc.conf`](https://github.com/petersulyok/smfc/blob/main/config/smfc.conf) (added the `control_function=` parameter to all temperature-based fan controller sections; bumped header from `smfc 5.x` to `smfc 6.x`); a new note 5 added under chapter 10.2 documenting the `control_function=` parameter.
- Default polling interval for the NVME fan controller lowered from `10` to `2` seconds (matches CPU/GPU defaults — NVMe SSDs can heat up quickly under load). Stale `default=10` comments in [`config/smfc.conf`](https://github.com/petersulyok/smfc/blob/main/config/smfc.conf), README chapter 10.2 and [`smfc-sample6.conf`](https://github.com/petersulyok/smfc/blob/main/config/samples/smfc-sample6.conf) corrected accordingly. The GPU fan controller `polling=` comment (which already used `2` as the value) was likewise corrected from `default=10` to `default=2`.
