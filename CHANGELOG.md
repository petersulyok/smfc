...
### Changed
- **`H14` BMC product names are not auto-detected as the `generic_x14` platform any more.** H14 boards have no per-zone manual fan mode - the OEM command answers `0xC1` - so `smfc` cannot control their fan levels through the X14 platform, and routing them there could only fail at startup. `auto` detection now matches `X14` only, and an H14 board falls back to `generic` like the H10-H13 boards. Setting `platform_name=generic_x14` explicitly still forces the X14 platform on any board. This reverts the `auto` detection change made in `smfc v6.0.0`.
