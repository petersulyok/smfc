...
- **A zone the BMC has forced to 100% after a fan failure is detected on X14/H14 boards.** `smfc` used to report it as fine; it now logs which zone is affected, and with `enforce_fan_mode=0` it stops with exit code 11 like any other loss of fan control.
