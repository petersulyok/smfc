...
### Added
- [doc/X14H14_MANUAL_FANCONTROL.md](https://github.com/petersulyok/smfc/blob/main/doc/X14H14_MANUAL_FANCONTROL.md): the IPMI raw command set of both 14th generation BMC firmware stacks, with the per-board fan sensor numbers and zone layouts, and how to tell which stack a board runs.
- **H14 boards are controllable.** `platform_name=generic_x14` is a platform *family* covering both 14th generation BMC firmware stacks: OpenBMC (most X14 boards plus `H14SHM`) and ATEN (every other H14 board plus the SoC boards `X14SDW`/`X14SDV`). The board name does not imply the stack, so it is settled by a runtime probe. On ATEN boards the lever is a global bypass flag rather than per-zone manual mode; see chapter 5 of the README for what that means for `ipmi_zone=`.

### Changed
- **The `generic_x14` platform reads which base fan modes the board supports** (`0x30 0x45 0x02`) instead of accepting the whole documented `0x00`-`0x0B` range. A mode a board does not have is accepted silently and reads back correctly while a different fan table is loaded, so the bitmask is the only way to know a mode is real. Firmware without the command keeps the old range.
- **The `generic_x14` platform discovers how many IPMI zones the board has.** It probes the per-zone failsafe flag at startup and stops at the first zone the board does not have, so an `ipmi_zone=` beyond that is rejected with a message naming the real count. The firmware bound is 5 zones (0-4).
...
### Fixed
- **Fan control was non-functional on X14/H14 motherboards (`generic_x14` platform).** Every command the platform issued was wrong in at least one byte. The OEM manual/failsafe commands are on netfn `0x2e` with IANA ID `cf c2 00`, and their reply echoes that ID back, so the flag is the last byte of `cf c2 00 <flag>`; on the wrong netfn the BMC rejects them with `0xC1`, which the stack probe read as "this board is ATEN". The fan level write is `0x30 0x70 0x66 0x01 <zone> <duty>`: selector `0x00` is the duty *read* and ignores a trailing duty byte, so the level `smfc` wrote never reached the fans and every zone stayed wherever the BMC's automatic curve had left it. The manual-mode commands also address zones 1-based while the duty commands address them 0-based, which the code did not account for. `smfc` now latches manual fan mode only in the zones it drives, reads the flag back to confirm it, and exits with code 8 naming the zone if the board does not accept it.
- **A zone the BMC holds at 100% is reported instead of looking healthy.** The failsafe flag outranks manual fan mode, so while it is set the BMC discards every duty `smfc` writes; only the manual-mode flag was polled, which stays latched throughout. A trip is now reported as a lost control state naming the zone and saying that restoring fan control cannot recover it.
- **`min_level=0` no longer reaches an OpenBMC board as a real 0%.** A manual duty write has no floor of its own - the 15% floor belongs to the automatic control a latched zone is no longer under - so writes clamp to 5%, as they already did on the ATEN stack.
