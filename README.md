...
If something else flips the BMC out of FULL mode while `smfc` is running (the BMC web UI, an external `ipmitool` invocation, a firmware quirk), `smfc` keeps issuing per-zone level commands but the BMC may apply its own profile instead — fans run at unintended speeds without any visible error in the log. To detect and react to this, `smfc` checks the BMC fan mode every loop iteration. The default `[Ipmi] enforce_fan_mode=1` logs the drift and re-asserts FULL plus all per-zone levels (some BMC firmwares reset zone levels when the mode changes); set it to `0` to make the service exit with code 11 instead, letting systemd restart the unit.
