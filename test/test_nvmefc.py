...
class TestNvmeFc:
    ...
    def test_init_p1(self, mocker: MockerFixture, count: int, ipmi_zone: List[int], temp_calc: int, steps: int,
        ...
        assert mynvmefc.config.ipmi_zone == ipmi_zone, error
        assert mynvmefc.name == cfg.section, error
        assert mynvmefc.count == count, error
        assert mynvmefc.config.temp_calc == temp_calc, error
        assert mynvmefc.config.steps == steps, error
        assert mynvmefc.config.sensitivity == sensitivity, error
        assert mynvmefc.config.polling == polling, error
        assert mynvmefc.config.min_temp == min_temp, error
        assert mynvmefc.config.max_temp == max_temp, error
        assert mynvmefc.config.min_level == min_level, error
        assert mynvmefc.config.max_level == max_level, error
