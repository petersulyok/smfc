...
| `platform_x14_openbmc` | `platform_name = generic_x14` in startup, resolving to `X14OpenBmcPlatform` (the Part 1.1 probe answers with a data byte); the zone 3 duty write `0x30 0x70 0x66 0x00 0x03 0x32` appears (CONST controller, 50%); the OEM manual-mode latch `0x2c 0x04 0xcf 0xc2 0x00 0x01 0x01 0x01` and its read-back `... 0x00 0x01` appear (note the 1-based zone byte); raw `0x30 0x45 0x01` (set fan mode) never appears — writing it would clear manual mode on every zone; the release `0x30 0x70 0x66 0x02 0x00` appears at exit. |
