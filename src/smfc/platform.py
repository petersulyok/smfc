...
class IpmiError(RuntimeError):
    ...
    unchanged. The completion code is what separates a command the BMC *rejected* (e.g. 0xC1 or 0xC7 -
    this command does not exist on this BMC stack) from an unreachable BMC, a wedged `/dev/ipmi0` or a
    `sudo` problem, all of which are otherwise indistinguishable by exception type. The X14/H14 stack
    probe depends on that distinction: any completion code means ATEN, `None` stays undetermined, and a
    wrong stack applies the wrong lever to the fans (see `doc/X14H14_MANUAL_FANCONTROL.md`, Part 1.3).
    """
