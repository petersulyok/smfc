...
class FanController:
    ...
    def _reuse_last_temp(self, index: int, error: Exception) -> float:
        ...
        error_tolerance budget of the device allows it, otherwise re-raise.
        ...
        if index >= len(self.last_per_device_temps) or errors > self.config.error_tolerance:
            self.log.msg(Log.LOG_ERROR, f"{self.name}: temperature read failed {errors} time(s) in a row "
                         f"(device={device}, error_tolerance={self.config.error_tolerance}): {error}")
            raise error
        temp = self.last_per_device_temps[index]
        self.log.msg(Log.LOG_ERROR, f"{self.name}: temperature read failed, reusing {temp:.1f}C "
                     f"(device={device}, {errors}/{self.config.error_tolerance}): {error}")
        return temp
    ...
    def get_temp(self) -> float:
        ...
        resets the counter of the device. See _reuse_last_temp() for the escalation path.
        ...
        for i in range(self.count):
            try:
                ...
                if self._temp_read_errors[i]:
                    self.log.msg(Log.LOG_INFO, f"{self.name}: temperature read recovered "
                                 f"(device={self.device_names()[i]})")
