...
class GenericX14Platform(Platform):
    ...
    The X14 BMC (OpenBMC based) uses different IPMI raw commands from the older platforms:
    - fan duty cycle control uses 0x30 0x70 0x88 (instead of 0x30 0x70 0x66),
    - manual mode must be enabled per zone via the OEM command 0x2c 0x04 0xcf 0xc2,
    - the duty cycle is a percentage (0x00-0x64), not the 0-255 scale,
    - extended fan modes 0x00-0x0B are supported.
    """

    FANCTL_COUNT: int = 6           # Number of fan zones (0-5)
